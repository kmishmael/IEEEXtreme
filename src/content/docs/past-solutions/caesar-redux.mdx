...
#### IEEEXtreme 17.0, [view on CSAcademy](https://csacademy.com/ieeextreme17/task/caesar_redux/statement/)

## Problem
We will walk through the process of solving a programming problem called "Caesar Redux." The problem involves implementing the Caesar cipher encryption technique to either encrypt or decrypt a given message based on specific conditions. We'll start by brainstorming how to approach the problem, what data structures we will use, and follow it with the code solution and its explanation.
...
- \(1 \leq n \leq 25\), where \(n\) is the number of messages.
